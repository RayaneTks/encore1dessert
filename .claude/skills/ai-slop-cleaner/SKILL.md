...
Checks (simulated):
1. Frontmatter includes `name: ai-slop-cleaner` and `level`.
2. Body has explicit “Preserve behavior” and “Write a cleanup plan before editing code”.
3. Has a bounded regression approach (tests-first, small diffs) and a `--review` reviewer-only posture.
4. No secrets: no tokens/keys in examples.

Simulated test:
- Run `python .claude/scripts/audit-skills-extract.py` and verify YAML parses; then grep the body for “deletion-first” and “--review”.
