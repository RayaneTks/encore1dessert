...
import { Trash2, ChevronDown, X, Layers, Award, LayoutDashboard } from 'lucide-react';
...
export const HistoryScreen: React.FC<Props> = ({ history, onDelete, showToast }) => {
  ...
  return (
    ...
    >
          ...
          {history.length === 0 ? (
            ...
            <div className="divide-y divide-gourmand-border/60">
              {history.slice(0, 20).map(h => (
                <button key={h.id} onClick={() => setSelectedEntry(h)} className="w-full p-4 flex justify-between items-center hover:bg-gourmand-bg/50 transition-colors text-left">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-end justify-center px-4 pb-12 sm:pb-32">
            ...
            <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} className="relative w-full max-w-[400px] bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden">
              ...
              <div className="p-5 space-y-5">
                ...
                <button onClick={() => { setSelectedEntry(null); setDeleteTarget(selectedEntry); }}
      ...
      <AnimatePresence>
        {deleteTarget && (
          <ConfirmDialog title="Annulation" message="Supprimer cette vente définitivement ?" onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} />
        )}
      </AnimatePresence>
    </motion.div>
  );
