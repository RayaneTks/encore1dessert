...
export const BasesScreen: React.FC<Props> = ({ bases, ingredients, setBases, showToast }) => {
  ...
  return (
    ...
    >
      <PageHeader
        title="Préparations"
        description={`${bases.length} base${bases.length > 1 ? 's' : ''} maison · Coûts recalculés dynamiquement`}
        action={
      ...
      <div className="px-4 space-y-4">
            ...
            <div key={base.id} className="gourmand-card overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-base text-gourmand-chocolate leading-tight">{base.name}</p>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <p className="font-semibold text-base text-gourmand-chocolate">{fmt(costPerKg)}</p>
                    <p className="text-[10px] font-medium text-gourmand-biscuit uppercase mt-0.5">/ kg</p>
                  </div>
                  ...
                  >
                    <div className="px-4 pb-4 space-y-4 pt-3 border-t border-gourmand-border">
                      ...
                      <div className="bg-gourmand-bg rounded-xl p-4 space-y-2">
                        <p className="text-[10px] font-semibold uppercase tracking-widest text-gourmand-biscuit mb-3">Composition technique</p>
                          ...
                          {base.components.map((comp, idx) => {
                            ...
                            return (
                              <div key={idx} className="flex justify-between items-center text-sm">
                                <span className="font-medium text-gourmand-chocolate">{ingName}</span>
                                <div className="flex items-center gap-3">
                        ...
                        <div className="border-t border-gourmand-border/60 pt-2.5 mt-3 flex justify-between items-center">
                          <span className="text-xs font-semibold text-gourmand-cocoa uppercase tracking-wide">Rendement : {totalWeight}g</span>
                          <span className="font-bold text-gourmand-chocolate">{fmt(totalCost)}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 pt-1">
                        <button
                          ...
                          className="flex-1 gourmand-btn-primary py-3"
                        >
                          Modifier la fiche
                        </button>
                        <button
        ...
        {bases.length === 0 && (
          <div className="text-center py-16 opacity-50">
            ...
            <p className="font-medium text-sm">Aucune préparation</p>
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {showForm && (
          <Modal onClose={() => setShowForm(false)} title={editItem ? 'Éditer la base' : 'Nouvelle base'}>
            <div className="p-5 space-y-5">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest text-gourmand-biscuit mb-1.5 ml-1">Nom de la préparation</p>
                <input
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest text-gourmand-biscuit mb-1.5 ml-1">Type de base</p>
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest text-gourmand-biscuit mb-3 flex items-center gap-1.5 ml-1">
                   <Apple size={14} /> Matières premières requises (g)
                </p>
              ...
              <textarea
                placeholder="Méthode ou notes techniques..."
              ...
              <button onClick={save} className="gourmand-btn-primary w-full py-4 text-sm mt-2">
                {editItem ? 'Enregistrer les modifications' : 'Créer la fiche technique'}
              </button>
            </div>
          </Modal>
        )}
